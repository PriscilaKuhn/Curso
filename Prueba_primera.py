...
def apuesta_ganadora(numeros_ganadores):
    if "apuesta" in datos and comparar_listas(datos["apuesta"], numeros_ganadores):
                print("¡Felicitaciones! Ganaste la Quiniela.")
                print(f"Los números ganadores de la fecha son : {numeros_ganadores}")
    else:
                print(f"Los números ganadores de la fecha son : {numeros_ganadores}")
                print("Lo siento, no ganaste. Mejor suerte la próxima.") #Procedimiento, imprime si la apuesta es o no ganadora.



def arqueo_de_caja():
    PORCENTAJE_GOBIERNO = 0.47
    ganancia_gobierno = datos['monto_recaudado']* PORCENTAJE_GOBIERNO
    ganancia_quiniela =datos['monto_recaudado'] - ganancia_gobierno

    print(f"Monto recaudado: {datos['monto_recaudado']} pesos")
    print(f"Ganancia para el Estado: {ganancia_gobierno}") 
    print(f"Ganancia para la Quiniela SANTO PANCRACIO: {ganancia_quiniela}")
    return ganancia_gobierno, ganancia_quiniela #Función, me permite calcular la recaudación diaria en apuestas y otros calculos más.
...
while eleccion != 5:
    ...
    if eleccion == 1:
        ...
        cargar_todo(eleccion,datos)
        mostrar(datos)
        imprimir=int(input("Si quiere volver a consultar el menú ingrese 0, si no cualquier otro número para seguir avanzando."))
        quiere_menu(imprimir)
        eleccion = int(input("Ingresar la opción: "))
    elif eleccion==3:
        ...
        ganancia_gobierno, ganancia_quiniela = arqueo_de_caja()
        imprimir=int(input("Si quiere volver a consultar el menú ingrese 0, si no cualquier otro número para seguir avanzando."))
        quiere_menu(imprimir)
        eleccion = int(input("Ingresar la opción: "))
    else:
